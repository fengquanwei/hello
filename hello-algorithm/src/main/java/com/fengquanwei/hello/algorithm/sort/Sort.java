package com.fengquanwei.hello.algorithm.sort;
...
public class Sort {
    ...
    public static void insertSort(int[] datas) {
        if (datas != null) {
            ...
            if (length > 1) {
                int i, j, t;
                for (i = 1; i < length; i++) {
                    if (datas[i] < datas[i - 1]) { // 如果当前元素大于前一元素，则不用变动位置
                        t = datas[i];
                        datas[i] = datas[i - 1];
    ...
    public static void bubbleSort(int[] datas) {
        if (datas != null) {
            ...
            if (length > 1) {
                int i, j, t, s = 1;
                for (i = 0; s == 1 && i < length - 1; i++) { // 冒泡 i 趟
                    s = 0; // 如果此趟没有交换，则不用进行下一趟了
    ...
    /**
     * 堆排序
     * 堆：一种完全二叉树（非终端结点的值均不大于或不小于其左右孩子结点的值）。
    ...
    /**
     * 归并排序
     * 归并是指将两个或两个以上的有序文件合并成为一个新的有序文件。
     * 归并排序是把一个有 n 个记录的无序文件看成是由 n 个长度为 1 的有序子文件组成的文件，然后进行两两归并，得到 n/2 个长度为 2 或 1 的有序文件，再两两归并，如此重复，直至最后形成包含 n 个记录的有序文件为止。这种反复将两个有序文件归并成一个有序文件的排序方法称为两路归并排序。
